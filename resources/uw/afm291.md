...
### IFRS FRAMEWORK DEFINITIONS

![ifrs framework](/resources/uw/images/framework.jpg){:width="100%"}

- users = "existing and potential investors, lenders, and other creditors," ie. those who provide financial resources to the enterprise
...
- fundamental qualitative characteristics (absolutely crucial):
...
- enhancing qualitative characteristics (not crucial, but makes the FS a lot easier to read and understand):
...
how to treat each element?

- definition = what to classify an item as (asset? liability? expense? etc.)
- recognition = when can the item be included in the company's books as something that is part of the entity (eg. when a building is purchased or built)?
...
- measurement = measuring the *monetary value* of an item
- presentation = where in the FS the item should be included (ie. in the FS, or in a footnote or disclosure?)
